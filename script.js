...
const mathConstsDropdownSummary = document.querySelector(
  "#mathConstantsDropdown>summary"
);
const mathConstsDropdownChildDiv = document.querySelector(
  "#mathConstantsDropdown>div"
);

function calculate(e) {
  if (e <= 9) {
    if (answer.toString().length > 0 && operatorId !== 0) {
    ...
    if (firstIn.length > 0) {
      if (operatorId === 0 && answer.toString().length === 0) {
    ...
    if (tCalcDisp.textContent.slice(-1) === "=") {
      btnClickAnim(delBtn);

      clrCalcDisp();
      clrData();
    } else if (
      tCalcDisp.textContent !== "Made by" &&
      bCalcDisp.textContent.length > 0
    ) {
      btnClickAnim(delBtn);
    ...
    if (bCalcDisp.textContent.length > 0 && bCalcDisp.textContent !== "‎ ") {
      clrCalcDisp();
    ...
    if (tCalcDisp.textContent !== "Made by" && secondIn.length > 0) {
...
function rmvUnccsryZrosAndDecsOnTail(i) {
  if ((i.toString().match(/\./g) || []).length === 1) {
    while (i.toString().slice(-1) === "0" || i.toString().slice(-1) === ".") {
      if (i === ".") {
        i = "0";
        return i;
      }
...
function setMathConstY() {
  ...
  mathConstantsDropdown.style.top =
    calculatorYPosition + calculatorHeight + 25 + "px";
